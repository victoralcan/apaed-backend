...
export default async function create(
  ...
  INSERT INTO NCM(id, description, long_description, minimal_qntt, ncm_code, type_id, unity_measurement_id) 
  values ('${ncm1Id}', 'Arroz Polido/Brunido não Parboilizado', 'É o mais comum entre os demais, também chamado de arroz branco ou tradicional', 
          '100', '1006.30.21', '${ncm1TypeId}', '${ncm1unityMeasurementId}');
  `);

  await connection.query(`
  INSERT INTO NCM(id, description, long_description, minimal_qntt, ncm_code, type_id, unity_measurement_id) 
  values ('${ncm2Id}', 'Feijão', 'Feijão comum', '200', '071333', '${ncm2TypeId}', '${ncm2unityMeasurementId}');
  `);

  await connection.query(`
  INSERT INTO NCM(id, description, long_description, minimal_qntt, ncm_code, type_id, unity_measurement_id) 
  values ('${ncm3Id}', 'Camisa de malha, masculina', 'Camisas de malha, de uso masculino', '10', '6105', '${ncm3TypeId}', '${ncm3unityMeasurementId}');
  `);

  await connection.query(`
  INSERT INTO NCM(id, description, long_description, minimal_qntt, ncm_code, type_id, unity_measurement_id) 
  values ('${ncm4Id}', 'Limpa Vidros Borrifador Veja', 'Limpa Vidro Borrifador VEJA 500ML BIO ÁLCOOL UNIT', 
          '10', '34022000', '${ncm4TypeId}', '${ncm4unityMeasurementId}');
  `);

  await connection.query(`
  INSERT INTO NCM(id, description, long_description, minimal_qntt, ncm_code, type_id, unity_measurement_id) 
  values ('${ncm5Id}', 'Short', 'Vestuário e seus acessórios, exceto de malha. Calças, jardineiras, bermudas e shorts (calções):', 
          '5', '6204.6', '${ncm5TypeId}', '${ncm5unityMeasurementId}');
  `);
